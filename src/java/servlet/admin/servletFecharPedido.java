 ...
 */
@WebServlet(name = "servletFecharPedido", urlPatterns = {"/servletFecharPedido"})
public class servletFecharPedido extends HttpServlet {
        ...
        try {
            ...
            if (verificausuarioestabelecimento.verificar(usuarioestabelecimento)) {
                try {
                    ...
                    dao.fechar(request.getParameter("id"),usuarioestabelecimento.getEstabelecimento());
                    response.sendRedirect("admin/detalhepedido.jsp?id=" + request.getParameter("id") + "&sucesso=1");
                } catch (Exception e) {
                    response.sendRedirect("admin/detalhepedido.jsp?id=" + request.getParameter("id") + "&erro=1");
                }
            } else {
